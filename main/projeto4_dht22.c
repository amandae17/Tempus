...
void DHT_reader_task(void *pvParameter)
{
	...
	while(1) {
		...
		int ret = readDHT(); // Realiza a leitura dos dados e retorna um status de erro 
		
		errorHandler(ret);

		printf("Humidity %.2f %%\n", getHumidity()); // Faz a leitura do valor da humidade.
		printf("Temperature %.2f degC\n\n", getTemperature()); // Faz a leitura do valor da temperatura.
		
		vTaskDelay(2000 / portTICK_PERIOD_MS); // Função que o faz delay de 2s para a leitura do sensor
	}
}

void app_main()
{
	nvs_flash_init(); // Utiliza a unidade de memória volátil para armazenar os dados de leitura.
    vTaskDelay(2000 / portTICK_PERIOD_MS); // Função que o faz delay de 2s para a leitura do sensor
	xTaskCreate(&DHT_reader_task, "DHT_reader_task", 2048, NULL, 5, NULL ); // Cria uma task para fazer a leitura do sensor
